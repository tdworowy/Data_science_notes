...
data_file_classification = path.join(
    path.dirname(path.abspath(__file__)), "../data/Social_Network_Ads.csv"
)
data_file_regression = path.join(
    path.dirname(path.abspath(__file__)), "../data/auto.csv"
)
data_file_weather = path.join(
    path.dirname(path.abspath(__file__)), "../data/weather.csv"
)
...
def get_data_weather() -> tuple:
    ...
    data_set["RainToday"] = data_set["RainToday"].apply(
        lambda x: 1 if x == "Yes" else 0
    )
    data_set["RainTomorrow"] = data_set["RainTomorrow"].apply(
        lambda x: 1 if x == "Yes" else 0
    )
